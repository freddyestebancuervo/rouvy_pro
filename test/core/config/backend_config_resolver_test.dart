...
void main() {
  group('override ausente/vacío — usa el valor del AppEnvironment (ambas políticas)', () {
    test('entorno que permite override, sin override: usa su valor predeterminado', () {
      final String result = resolveBackendBaseUrl(_overridableEnvironment, override: '');

      expect(result, 'http://environment-default.example/v1');
    });

    test('entorno que permite override, override vacío tras trim: usa su valor predeterminado', () {
      final String result = resolveBackendBaseUrl(_overridableEnvironment, override: '   ');

      expect(result, 'http://environment-default.example/v1');
    });

    test('entorno que NO permite override, sin override: usa su valor predeterminado', () {
      final String result = resolveBackendBaseUrl(_lockedEnvironment, override: '');

      expect(result, 'https://environment-default-prod.example/v1');
    });
  });

  group('Development (allowsBackendOverride: true) — override aceptado si es válido', () {
    ...
    test('override inválido lo rechaza (falla de formato, no de política)', () {
      expect(
        () => resolveBackendBaseUrl(_overridableEnvironment, override: 'no-es-una-url'),
        throwsStateError,
      );
    });
    ...
    test('preserva la URL exacta del override, sin normalizarla', () {
      ...
      final String result = resolveBackendBaseUrl(_overridableEnvironment, override: override);

      expect(result, override);
    });
  });

  group('Production (allowsBackendOverride: false) — override siempre rechazado', () {
    test('override válido (URL bien formada) igual se rechaza por política de entorno', () {
      expect(
        () =>
            resolveBackendBaseUrl(_lockedEnvironment, override: 'https://otro-backend.example/v1'),
        throwsStateError,
      );
    });
    ...
    test('override inválido también se rechaza por política de entorno', () {
      expect(
        () => resolveBackendBaseUrl(_lockedEnvironment, override: 'no-es-una-url'),
        throwsStateError,
      );
    });

    test('el mensaje de excepción explica claramente que Production no admite override', () {
      try {
        resolveBackendBaseUrl(_lockedEnvironment, override: 'https://otro-backend.example/v1');
    ...
    test('un override prohibido nunca cae en silencio al valor del entorno — falla, no continúa',
        () {
  ...
  group('productionEnvironment REAL (no un doble sintético) — prueba de inicialización', () {
    test(
      ...
      () {
        expect(
          ...
          throwsA(
            isA<StateError>()
                .having((StateError e) => e.message, 'message', contains('production'))
                .having(
  ...
  group('resolveBackendBaseUrl — validación de formato de URL (aplica a ambas políticas)', () {
    test('rechaza cadena vacía después de trim (valor del propio entorno mal configurado)', () {
    ...
    test('rechaza un esquema distinto de http/https', () {
      expect(
        () => resolveBackendBaseUrl(_overridableEnvironment, override: 'ftp://backend.example/v1'),
        throwsStateError,
      );
    });
    ...
    test('rechaza una URL sin host', () {
      expect(
        () => resolveBackendBaseUrl(_overridableEnvironment, override: 'http:///v1'),
        throwsStateError,
      );
    });
  });
