...
- **Actualizado por:** Ejecutor técnico documental (cierre de T-F0.2 Bloque 2 — conexión de Development al backend real)
- **Commit base de `main` auditado al iniciar este bloque:** `99504dbb2719dc1d26a516132b9eaf2489b76e38`
...
- **`T-F0.2`/`C1`** — separación completa de entornos Firebase (Development/Staging/Production): **abierta — infraestructura base completada; código de bootstrap y backend de Development conectados; cierre formal pendiente de revisión de la matriz de entornos**. `ridepro-development` ya dispone de: Firestore creado en `southamerica-east1`; bucket regional de Storage (`ridepro-development.firebasestorage.app`, vacío, creado manualmente y de forma autorizada por el propietario el 2026-08-02 después del PR #20) con `storage.rules` deny-by-default **desplegadas y verificadas en producción** (prueba anónima real → `403`); facturación vinculada con un presupuesto de alertas configurado (50%/90%/100%, sin ser un límite automático de gasto); Authentication con Email/Password, Google y Apple habilitados según verificación visual del propietario en Console (Teléfono y Anónimo no figuraban habilitados en esa verificación visual, MFA por SMS no figuraba habilitado). **Código:** `AppEnvironment`/`bootstrapRideProApp`/`main_development.dart` integrados a `main` (PR #22, Bloque 1); `environment_development.dart.backendBaseUrl` conectado al backend Cloud Run/Cloud SQL real de Development (Bloque 2, implementado y validado en la rama `feat/tf02-dev-backend-wiring-20260802` — **pendiente de PR y fusión**, ver historial). Salvedades que **no** se dan por cerradas: configuración integral de Apple Developer (independiente de Firebase), etiqueta comercial exacta "Blaze" (sin confirmación visual explícita de esa palabra), **matriz formal de entornos documentada (backend/DB/Firebase/CORS/logs por entorno, criterio de aceptación oficial de `C1` en `BACKLOG_MAESTRO.md`) — todavía no existe como documento único**, entorno Staging (sin iniciar) y trabajo adicional aún no integrado desde `feature/d2` (decisiones D1-D8 ya aprobadas por el propietario). Detalle completo en `docs/audits/AUDITORIA_FINAL/17_CIERRE_FIRESTORE_RIDEPRO_DEVELOPMENT.md` secciones 11-12.
...
Con `T-F0.1`, la cobertura de tests de Auth, la validación de plataforma iOS (scaffold + Firebase + bundling del plist + inicialización en runtime) ya integrados a `main` (PRs #9-#17), y `T-F0.2`/`C1` con su código de bootstrap en `main` (PR #22) y el backend de Development ya conectado (Bloque 2, ver historial), el orden priorizado para el trabajo posterior es:

1. **`T-F0.2`/`C1` — matriz formal de entornos**: redactar el documento único que consolide backend/DB/Firebase/CORS/logs por entorno (criterio de aceptación oficial de `C1`, `BACKLOG_MAESTRO.md`) — es lo único que falta para poder evaluar el cierre formal de `C1`. En paralelo, decisiones aún pendientes con el propietario: si/cuándo desplegar Hosting de Development (Documento 20, Fase 3 — sin ejecutar), si/cuándo configurar más proveedores de Authentication, si/cuándo crear el entorno Staging.
