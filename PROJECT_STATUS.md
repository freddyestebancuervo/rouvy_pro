...
- **`T-F0.2`/`C1`** — separación completa de entornos Firebase (Development/Staging/Production): **abierta — infraestructura base completada; cierre formal pendiente de revisión de criterios**. `ridepro-development` ya dispone de: Firestore creado en `southamerica-east1`; bucket regional de Storage (`ridepro-development.firebasestorage.app`, vacío, creado manualmente y de forma autorizada por el propietario el 2026-08-02 después del PR #20) con `storage.rules` deny-by-default **desplegadas y verificadas en producción** (prueba anónima real → `403`); facturación vinculada con un presupuesto de alertas configurado (50%/90%/100%, sin ser un límite automático de gasto); Authentication con Email/Password, Google y Apple habilitados según verificación visual del propietario en Console (Teléfono y Anónimo no figuraban habilitados en esa verificación visual, MFA por SMS no figuraba habilitado). Salvedades que **no** se dan por cerradas: configuración integral de Apple Developer (independiente de Firebase), etiqueta comercial exacta "Blaze" (sin confirmación visual explícita de esa palabra), matriz completa de entornos (Staging/Production) y trabajo adicional aún no integrado desde `feature/d2` (decisiones D1-D8 ya aprobadas por el propietario). Detalle completo en `docs/audits/AUDITORIA_FINAL/17_CIERRE_FIRESTORE_RIDEPRO_DEVELOPMENT.md` secciones 11-12.
...
1. **`T-F0.2`/`C1` — decisiones pendientes de `ridepro-development`**: definir con el propietario, sin ejecutar todavía, si/cuándo desplegar `storage.rules` contra el proyecto real; por separado, definir si/cuándo configurar los proveedores de Authentication y, también por separado, verificar posteriormente su estado mediante Firebase Console o acceso de solo lectura con mínimo privilegio ya autorizado, sin ampliar IAM únicamente para esta auditoría; y si/cuándo crear presupuestos de facturación — ninguna de estas acciones tiene autorización para ejecutarse en este momento. En paralelo, formalizar la matriz completa de entornos (Staging/Production), que sigue sin iniciarse.
